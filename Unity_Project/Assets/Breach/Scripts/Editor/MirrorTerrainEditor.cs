    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Mirror X-axis"))
        {
            ...
            for (int i = 0; i < xLen; i++)
            {
                ...
                {
                    newHeights[yLen - j - 1, xLen - i - 1] = mirrorHeights[j, xLen - i - 1];
                }
            }

            terComponent.terrainData.SetHeights(0, 0, newHeights);
        }
        ...
        if (GUILayout.Button("Mirror Y-axis"))
        {
            ...
            for (int i = 0; i < xLen; i++)
            {
                for (int j = 0; j < yLen; j++)
                {
                    newHeights[yLen - j - 1, xLen - i - 1] = mirrorHeights[yLen - j - 1, i];
